...
public class ShellSort {
    public static void main(String[] args) {
        /*int[] arr = { 8, 9, 1, 7, 2, 3, 5, 4, 6, 0 ,78,98,99,101,30};
        shellSort2(arr);
    ...
    // 希尔排序时， 对有序序列在插入时采用交换法实现（再确定步长后的排序相当于冒泡排序）
    ...
    //对交换式的希尔排序进行优化->移位法（再确定步长后的排序向当于插入排序）
    public static void shellSort2(int[] arr) {
        ...
        for (int gap = arr.length / 2; gap > 0; gap /= 2) {
            ...
            for (int i = gap; i < arr.length; i++) {
                int j = i;
                int temp = arr[j];
                //若是arr[j] >= arr[j - gap],就说明这个无序数正好大于有序列表的最后一位，则此数不需要排序
                if (arr[j] < arr[j - gap]) {
